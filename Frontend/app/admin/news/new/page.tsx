  ...
  const onSubmit = async (data: NewsFormData) => {
    ...
    try {
      ...
      if (imageUrl && !response.data.image) {
        ...
        toast.error('News created but image was not saved. Please edit the news to add the image.')
      } else if (imageUrl && response.data.image) {
        console.log('Image successfully saved:', response.data.image)
      }
      
      if (response.data.status === 'draft') {
        toast.success('News created as draft. Change status to "published" to show on public site.')
      } else {
      ...
      const errorMessage = error.response?.data?.error || error.message || 'Failed to create news'
